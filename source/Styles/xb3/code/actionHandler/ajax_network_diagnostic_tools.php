...
if (isset($_POST['test_connectivity'])){
	...
	// ccsp_setStr("Device.IP.Diagnostics.IPPing.Interface","Device.IP.Interface.1");
	ccsp_setStr("Device.IP.Diagnostics.IPPing.Host",$destination_address,true);
	ccsp_setStr("Device.IP.Diagnostics.IPPing.NumberOfRepetitions",$count1,true);
	ccsp_setStr("Device.IP.Diagnostics.IPPing.DiagnosticsState",$DiagnosticsState,true);
	do{
		sleep(1);
		$pingState=ccsp_getStr("Device.IP.Diagnostics.IPPing.DiagnosticsState");
	}while(!in_array($pingState,$states));
	$success_received=ccsp_getStr("Device.IP.Diagnostics.IPPing.SuccessCount");
	// $failure_received=ccsp_getStr("Device.IP.Diagnostics.IPPing.FailureCount");
	...
	$count2=$_POST['count2'];
	$DiagnosticsState="Requested";
	ccsp_setStr("Device.IP.Diagnostics.IPPing.Host",$destination_ipv4,true);
	ccsp_setStr("Device.IP.Diagnostics.IPPing.NumberOfRepetitions",$count2,true);
	ccsp_setStr("Device.IP.Diagnostics.IPPing.DiagnosticsState",$DiagnosticsState,true);
	do{
		sleep(1);
		$pingState=ccsp_getStr("Device.IP.Diagnostics.IPPing.DiagnosticsState");
	...
	$count3=$_POST['count3'];
	$DiagnosticsState="Requested";
	ccsp_setStr("Device.IP.Diagnostics.IPPing.Host",$destination_ipv6,true);
	ccsp_setStr("Device.IP.Diagnostics.IPPing.NumberOfRepetitions",$count3,true);
	ccsp_setStr("Device.IP.Diagnostics.IPPing.DiagnosticsState",$DiagnosticsState,true);
	do{
		sleep(1);
		$pingState=ccsp_getStr("Device.IP.Diagnostics.IPPing.DiagnosticsState");
	...
	ccsp_setStr("Device.IP.Diagnostics.TraceRoute.Host", $trace_ipv4_dst, true);
	ccsp_setStr("Device.IP.Diagnostics.TraceRoute.DiagnosticsState", $trace_ipv4_status, true);
	do{
		sleep(3);
		$trace_ipv4_status = ccsp_getStr("Device.IP.Diagnostics.TraceRoute.DiagnosticsState");
		// $trace_ipv4_status = "Complete";
	}while(!in_array($trace_ipv4_status, $states_trace));
	
	if ("Complete" == $trace_ipv4_status){
		$ids = explode(",", ccsp_getInstanceIds("Device.IP.Diagnostics.TraceRoute.RouteHops."));
		foreach($ids as $i){
			$time = ccsp_getStr("Device.IP.Diagnostics.TraceRoute.RouteHops.$i.RTTimes");
			$host = ccsp_getStr("Device.IP.Diagnostics.TraceRoute.RouteHops.$i.Host");
			$addr = ccsp_getStr("Device.IP.Diagnostics.TraceRoute.RouteHops.$i.HostAddress");
	...
	ccsp_setStr("Device.IP.Diagnostics.TraceRoute.Host", $trace_ipv6_dst, true);
	ccsp_setStr("Device.IP.Diagnostics.TraceRoute.DiagnosticsState", $trace_ipv6_status, true);
	do{
		sleep(3);
		$trace_ipv6_status = ccsp_getStr("Device.IP.Diagnostics.TraceRoute.DiagnosticsState");
